...
${agentOn ? '【代理模式開啟】你能操控 App 並直接修改這份行程：navigate、open_day、show_on_map、open_google_maps、show_souvenirs、find_hotels；add_activity／remove_activity／update_activity／move_activity 調整單一活動；add_day／remove_day 增減天數；reset_plan 還原。\n「從零規劃整趟」：使用者說「幫我規劃去○○、玩○天」時呼叫 plan_trip（在 request 中盡量帶齊：目的地、天數或起訖日期、出發地/機場、班機時間、偏好、預算、同行）。plan_trip 會自動產生「像專業旅遊書一樣完整」的行程（每城多個景點、每天 5–8 個含時間/停留/花費的活動、城市間交通、交通票、預算、打包、各城伴手禮、當地緊急電話）。完成後務必呼叫 open_day 1 讓使用者看到行程，並用 2–3 句說明重點與亮點。關鍵資訊不足時先用文字逐項問清楚（日期、機場、班機時間）再規劃。複雜調整可連續呼叫多個工具。' : '【代理模式關閉】僅以文字回答；若使用者想調整或從零建立行程，建議他開啟上方代理模式。'}
回答精簡（3–6 句），善用條列。\n\n${tripContext()}`;
}
...
否則回傳「完整」行程 JSON（所有人類可讀文字一律繁體中文）：
{
  ...
  for (let i = 0; i < 6; i++) {
    ...
    try {
      ...
      const msg = e.message === 'NO_KEY'
        ...
        ? '⚠️ AI 用量已達上限（配額／速率限制）。請稍等一兩分鐘再試；若經常發生，可到 Google AI Studio 確認方案與配額，或在「設定」改用自己的 API 金鑰。'
        : '⚠️ 連線發生問題：' + e.message;
      addAI(scroll, msg);
      return;
    }
