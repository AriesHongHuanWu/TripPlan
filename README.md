...
- **行程詳情頁 + 見解/心得**：點社群卡片進入詳情頁（英雄區＋作者＋♥/🔁/★/💬 統計＋行程／心得分頁，可用 `?view=<code>` 連結直接分享）。登入後可**評分 1–5 星＋寫心得**（每人一篇、可編輯刪除）、對別人的心得按「👍 有幫助」。資料在 `feed/{code}/reviews/{uid}`；評分數與星等以**交易原子方式**累計到 `feed` 文件，規則綁定逐人 doc 防偽造。
- 社群只暴露一份**安全的公開中繼資料** `feed/{code}`（標題/國家/區域/標籤/天數…），**不含**可編輯的行程內容、成員或 email；完整內容仍只存在受權限保護的 `collab/{code}`。
> 社群＋見解都由 `firestore.rules` 在伺服器端強制——**每次更新規則後務必重新發布**（現含 `feed/{code}` + `reviews/{uid}` + `helpful`），否則社群讀寫／寫心得會被拒。`feedList`／`reviewList` 只用單欄位排序，**不需建立複合索引**。
